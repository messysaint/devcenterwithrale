        ...
        try {
            ...
            try {
                while( (s = in.readLine())!= null ) {
                    ...
                    } else if( s.toUpperCase().indexOf( "SSHVERSION" ) != -1 ) {
                        sshVersion = s.substring( s.indexOf( '=')+1, s.length() ).trim();
                        key = sshVersion.substring(0, keyLen);
                        sshVersion = sshVersion.substring(keyLen);
                        sshVersion = psec.decryptFromBase64( sshVersion, key );
                    }
                }
