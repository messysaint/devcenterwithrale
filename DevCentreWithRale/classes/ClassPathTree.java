...
public class ClassPathTree extends JPanel {
    ...
    public ClassPathTree(mainForm parent) {
        ...
        rootNode = new DefaultMutableTreeNode("Project ClassPath");
    ...
    public void refreshTree( String[] jarFiles ) {
        ...
        if( libChanged( jarFiles ) ) {
            ...
            for( int i = 0 ; i < jarFiles.length ; i++ ) {
                ...
                jarNodes[i]  = new DefaultMutableTreeNode( jar );                
    ...
    public boolean libChanged( String[] jarFiles ) {
        ...
        for( int i = 0 ; i < jarFiles.length ; i++ ) {
            jarFiles[i] = jarFiles[i].substring( jarFiles[i].lastIndexOf( '\\' )+1 );
        }
    ...
    public String getLibPath( String jarName ){
        ...
        main.getMountedJar();        
        jars = main.getMountedJarProperties();
        
        for( int i = 0 ; i < jars.length ; i++ ) {
            if( jars[i].endsWith( jarName ) ) {
                rvalue = jars[i];
                break;
            }
        }
